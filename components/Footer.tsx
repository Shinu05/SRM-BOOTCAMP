...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-secondary border-t border-border pt-16 pb-8 text-foreground mt-auto">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          ...
          <div className="flex flex-col items-start col-span-1 md:col-span-1">
            <Link href="/" className="mb-4 hover:opacity-80 transition-opacity">
              <Image src="/logo.jpg" alt="Atlas Logo" width={160} height={160} className="object-cover w-14 h-14 rounded-full shadow-sm" />
            </Link>
            <p className="text-sm text-muted max-w-xs font-medium mb-6">
              Explore. Shop. Elevate.<br />
              Premium essentials for the modern lifestyle.
            </p>
            <div className="flex items-center gap-5 text-muted">
              <a href="#" className="hover:text-foreground hover:-translate-y-1 transition-all" aria-label="Twitter">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                </svg>
              </a>
              <a href="#" className="hover:text-foreground hover:-translate-y-1 transition-all" aria-label="Facebook">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
                </svg>
              </a>
              <a href="#" className="hover:text-foreground hover:-translate-y-1 transition-all" aria-label="Instagram">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2.5 text-sm text-muted font-medium">
              ...
              <li><Link href="/products?category=Wearables" className="hover:text-foreground transition-colors">Wearables</Link></li>
              <li><Link href="/products?category=Bags" className="hover:text-foreground transition-colors">Everyday Carry</Link></li>
            </ul>
          </div>

          <div>
            <h4 className="font-semibold text-foreground mb-4 text-sm">About Atlas</h4>
            <ul className="flex flex-col gap-2.5 text-sm text-muted font-medium">
        ...
        <div className="pt-6 border-t border-border/60 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted font-medium">
            &copy; {currentYear} Atlas Inc. All rights reserved.
          </p>
          <div className="flex gap-4 md:gap-6 text-xs text-muted font-medium">
